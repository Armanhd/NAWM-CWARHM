...
"""
Calculate soil-class occurrence for a one-HRU lumped basin.

Reuses the existing distributed soil raster and maps it to:
...
Output:
    ...
    domain_<DOMAIN>/lumped/shapefiles/
        catchment_intersection/with_soilgrids/
...
if len(sys.argv) != 2:
    raise SystemExit(
        "Usage:\n"
        "python 2_find_HRU_soil_classes_lumped.py "
...
# ============================================================
# REUSE EXISTING DISTRIBUTED SOIL RASTER
# ============================================================
...
if not soil_file.exists():
    ...
    raise FileNotFoundError(
        "Existing distributed soil raster not found:\n"
        f"{soil_file}"
    )
...
try:
    ...
    output_name = "catchment_with_soilgrids.shp"
...
with rasterio.open(
    ...
    if (
        ...
        raise RuntimeError(
            "Lumped catchment and soil raster do not overlap."
        )
...
if not np.allclose(
    ...
    raise RuntimeError(
        "Soil classes are not integer categorical values."
    )
...
if len(stats) != 1:
    raise RuntimeError(
        "Expected one soil zonal-statistics result."
    )
...
for value in unique_values:
    ...
    if len(field) > 10:
        raise RuntimeError(
            f"Shapefile field name exceeds 10 characters: {field}"
        )
...
if total_pixels <= 0:
    raise RuntimeError(
        "No valid soil pixels were assigned to the lumped HRU."
    )
...
if len(saved) != 1:
    raise RuntimeError(
        "Saved soil output does not contain one HRU."
    )


if int(saved.iloc[0]["HRU_ID"]) != 1:
    raise RuntimeError(
        "Saved soil HRU_ID is not 1."
    )


if int(saved.iloc[0]["GRU_ID"]) != 1:
    raise RuntimeError(
        "Saved soil GRU_ID is not 1."
    )
...
if not saved_fields:
    raise RuntimeError(
        "No USGS fields found in saved soil output."
    )
...
if saved_total <= 0:
    raise RuntimeError(
        "Saved soil histogram contains no pixels."
    )
...
log_file = (
    log_dir
    / f"{now:%Y%m%d_%H%M%S}_lumped_soil.txt"
)
...
print()
print("=" * 70)
print("LUMPED HRU SOIL-CLASS PROCESSING COMPLETED")
print("=" * 70)
...
print(f"Output       : {output_file}")
print()
print(f"PASS: {domain} lumped soil classes")