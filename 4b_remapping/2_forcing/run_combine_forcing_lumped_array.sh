#!/bin/bash
#SBATCH --job-name=lumped_combine
#SBATCH --time=08:00:00
#SBATCH --mem=4G
#SBATCH --cpus-per-task=1
#SBATCH --output=slurm_logs/lumped_combine_%A_%a.out
#SBATCH --error=slurm_logs/lumped_combine_%A_%a.err
...
PYTHON_SCRIPT="${SCRIPT_DIR}/3_combine_forcing_for_SUMMA_lumped.py"
...
if [ "$#" -lt 1 ] || [ "$#" -gt 2 ]; then
    ...
    echo "    run_combine_forcing_lumped_array.sh \\"
...
if [ ! -f "${PYTHON_SCRIPT}" ]; then
    echo "ERROR: Lumped forcing-combination script not found:"
    echo "${PYTHON_SCRIPT}"
    exit 1
fi
...
echo
echo "======================================================================"
echo "LUMPED SUMMA FORCING COMBINATION - CHUNKED"
echo "======================================================================"
echo
...
for TASK_INDEX in $(seq "${START_INDEX}" "${END_INDEX}"); do
    ...
    if [ -z "${TASK_LINE}" ]; then
        echo
        echo "ERROR: No task found at line ${LINE_NUMBER}"
        FAILED=$((FAILED + 1))
        continue
    fi
    ...
    if [ "${MONTH}" -lt 1 ] || [ "${MONTH}" -gt 12 ]; then
        echo "ERROR: Month outside 1-12: ${MONTH}"
        FAILED=$((FAILED + 1))
        continue
    fi
...
echo
echo "======================================================================"
echo "LUMPED SUMMA FORCING COMBINATION CHUNK COMPLETED"
echo "======================================================================"
echo
echo "Array task ID : ${SLURM_ARRAY_TASK_ID}"
