#!/bin/bash
#SBATCH --job-name=lumped_emearth_remap
#SBATCH --time=08:00:00
#SBATCH --mem=4G
#SBATCH --cpus-per-task=1
#SBATCH --output=slurm_logs/lumped_emearth_remap_%A_%a.out
#SBATCH --error=slurm_logs/lumped_emearth_remap_%A_%a.err

# ============================================================
# LUMPED MULTIBASIN EM-EARTH REMAPPING - CHUNKED SLURM WORKER
# ============================================================
#
# Purpose
# -------
# Remap prepared EM-Earth monthly forcing to the one-HRU
# lumped SUMMA catchment.
#
# One Slurm array element processes multiple basin-month tasks.
#
# Task-file format:
#
...
#   CHUNK_SIZE=500
#
#   N=$(wc -l < lumped_month_tasks_BATCH.txt)
#
#   NCHUNKS=$(( (N + CHUNK_SIZE - 1) / CHUNK_SIZE ))
#
#   sbatch \
#       --array=0-$((NCHUNKS-1)) \
#       run_remap_EM_Earth_lumped_array.sh \
#       lumped_month_tasks_BATCH.txt \
#       "$CHUNK_SIZE"
#
# IMPORTANT
# ---------
...
#   - reuses distributed prepared EM-Earth forcing
#   - writes outputs under domain_<DOMAIN>/lumped/
#   - does NOT use control_active.txt
#   - does NOT modify distributed outputs
#
# ============================================================
...
PYTHON_SCRIPT="${SCRIPT_DIR}/2b_remap_all_EM_Earth_lumped.py"
...
if [ "$#" -lt 1 ] || [ "$#" -gt 2 ]; then
    ...
    echo "    run_remap_EM_Earth_lumped_array.sh \\"
...
if [ ! -f "${PYTHON_SCRIPT}" ]; then
    ...
    echo "ERROR: Lumped EM-Earth remapping script not found:"
    echo "${PYTHON_SCRIPT}"
...
echo
echo "======================================================================"
echo "LUMPED MULTIBASIN EM-EARTH REMAPPING - CHUNKED"
echo "======================================================================"
echo
...
for TASK_INDEX in $(seq "${START_INDEX}" "${END_INDEX}"); do
    ...
    if [[ "${CONTROL_FILE}" != *_lumped.txt ]]; then
        ...
        echo
        echo "ERROR: Non-lumped control supplied:"
        echo "${CONTROL_FILE}"
...
echo
echo "======================================================================"
echo "LUMPED EM-EARTH REMAPPING CHUNK COMPLETED"
echo "======================================================================"
echo
echo "Array task ID : ${SLURM_ARRAY_TASK_ID}"
